...
public:
    ...
    virtual void updata_file();                              // 更新信息--管理员密码，用户信息
    ...
    void delete_people();                                    // 删除指定用户
    void delete_people_all();                                // 删除所有用户
    //
    void change_account_people(const int i, const char num[20]) { strcpy(m_people_a[i], num); }  // 修改指定用户账号
    void change_password_people(const int i, const char num[20]) { strcpy(m_people_p[i], num); } // 修改指定用户密码
private:
        ...
        case 1: // 登录管理员系统
        {
            ...
            while (1)
            {
                ...
                case 1: // 显示系统信息
                {
                    ...
                    if (user_manager.Get_total() != 0)
                    {
                        cout << "用户账号如下:" << endl;
                        for (int total_i = 1; total_i <= user_manager.Get_total(); total_i++)
                        {
                            cout << user_manager.Get_account(total_i) << endl;
                        }
                    }
                    break;
                }
...
int Manager::change_password()
{
    ...
    memset(password_old, '\0', 20);
    memset(password_change, '\0', 20);
    memset(password2_change, '\0', 20);
    cout << "请输入密码进行身份确定：" << endl;
    while (true)
    {
        ch = _getch();  // 无回显的输入
...
{
    int i = 0, j = 5;
    char ch, account_logon[20], account2_logon[20], password_logon[20], password2_logon[20];
    memset(account_logon, '\0', 20);
    memset(account2_logon, '\0', 20);
    memset(password_logon, '\0', 20);
    memset(password2_logon, '\0', 20);
    while (i <= m_people_total)
    {
        cout << "请输入账号：" << endl;
        cin >> account_logon;
        strcpy(account2_logon, account_logon);
        for (i = 1; i <= m_people_total; i++)
        {
    ...
    while (j--)
    {
        ...
        if (strcmp(password_logon, password2_logon) == 0)
        {
            ...
            out_manager.close();
            ofstream out_people(strcat(account_logon, ".txt"), ios::trunc);
            if (out_people)
            {
                out_people << account2_logon << " " << password_logon << " 0" << endl;
            }
            out_people.close();
            return;
        }
...
int People::change_account_password(Manager *manager)
{
    ...
    memset(password_change, '\0', 20);
    memset(password2_change, '\0', 20);
    memset(account_change, '\0', 20);
    memset(account_old, '\0', 20);
    int choose_change, i = 0, flag = 0;
    strcpy(account_old, account);
    cout << "请输入密码进行身份确定：" << endl;
    while (true)
    {
        ch = _getch();  // 无回显的输入
    ...
    if (strncmp(password_old, password, 20) == 0)
    {
        ...
        case 2: // 修改密码
        {
            while (1)
            {
                ...
                {
                    if (strcmp(password_change, password2_change) == 0)
                    {
                        cout << "密码修改成功！" << endl;
                        cout << "登录失效，请重新登录！" << endl;
                        break;
